...
**B. Autonomous multi-file refactor with validation (writes, isolated):**
```bash
TO 240 droid exec "Refactor the auth module from callbacks to async/await, then run 'npm test' and fix failures. Report files changed." --auto medium --output-format json 2>/dev/null
```
...
**F. Multi-agent mission with cross-validation (write, isolated worktree):**
```bash
TO 300 droid exec "Add integration tests covering the order flow and make them pass." --mission --auto high --validator-model gpt-5 --worktree add-order-tests --output-format json 2>/dev/null
```
